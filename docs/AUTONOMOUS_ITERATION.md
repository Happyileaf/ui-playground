...
为确保创新方向符合项目定位，避免同质化和过度学术化，完成初始idea产生后，必须进行一轮独立评审和约束检查：

1. **重复性评审**
   - 完整读取 `src/playground.js` 中所有已注册案例的 `id`、标题、描述
   - 确认本次idea与任何已有案例都**没有本质重复**，确属全新技术域拓展
   - 如果只是微小调整或体验优化，应归入「模式 B：现有案例体验升维」，而非全新拓展

2. **内容方向评审**
   - 🎯 **优先方向**：有用户交互（点击/悬停/拖拽/滚动）、视觉美观、适合实际网页使用的交互特效和 UI 组件
   - ⚠️ **注意平衡**：纯数学生成艺术/物理模拟类效果也很有创意，但项目整体需要保持平衡，不要全是这类内容
   - ❌ **绝对禁止**：无任何交互的纯自动播放动画、会导致浏览器卡死的密集计算循环

3. **ID 命名规范检查**
   - 案例 ID 必须使用小写字母和连字符（kebab-case），例如 `magnetic-button`
   - ID 必须简洁清晰，能够准确描述核心效果，避免过于笼统或冗长
   - ID 不能与任何已有案例重复，也不能使用已删除/废弃的旧 ID
